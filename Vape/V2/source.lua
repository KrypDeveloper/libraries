...
function lib:Window(tabel)
    local closebind = tabel.KeyCode
    local preset = tabel.Color
    local text = tabel.Name
    if tabel.KeySettings.Active == true then
    ...
    Sentinel:Create({
        ServiceName = tabel.Name -- Script Name
        ServiceID = tabel.Keysystem.Service -- PandaAuth's Service Identifier
        AutoLogin = tabel.Keysystem.AutoLogin -- True or False. if true, it automatically login the user without pressing the login button, if false then user needs to click the login button after authorization.
        Callback = function() 
            getgenv().AlredyPassed = true 
        end -- Runs after successfully authorized 
    })
    if getgenv().Keysystem == true and getgenv().AlredyPassed == false then
        repeat wait() until getgenv().AlredyPassed == true
    end
    Main.Name = "Main"
    Main.Parent = ui
